...
async function main() {
    ...
    router.post('/api/art', async (ctx) => {
        ...
        const response = await artService.create({...ctx.request.body});
        ctx.body = response;
    });
    router.put('/api/art/:id', async (ctx) => {
        logger.info(`PUT Request:`, { message: ctx.URL });
        
        const response = await artService.updateById(ctx.params.id, {...ctx.request.body});
        ctx.body = response;
    });
    ...
    router.post('/api/jewelry', async (ctx) => {
        ...
        const response = await jewelryService.create({...ctx.request.body});
        ctx.body = response;
    });
    router.put('/api/jewelry/:id', async (ctx) => {
        logger.info(`PUT Request:`, { message: ctx.URL });
        
        const response = await jewelryService.updateById(ctx.params.id, {...ctx.request.body});
        ctx.body = response;
    });
